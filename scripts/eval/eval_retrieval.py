...
def get_score(answer_doc, topk=20):
	...
	for p in docs:
		topkpara_covered.append(
			int(para_has_answer(answer, p["title"] + " " + p["text"], PROCESS_TOK))
		)
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"--unified", action="store_true", help="test with unified trained model"
	)
	...
	for b_start in tqdm(range(0, len(questions), args.batch_size)):
		with torch.no_grad():
			...
			for b_idx in range(len(batch_q)):
				topk_docs = [
					{"title": id2doc[str(doc_id)][0], "text": id2doc[str(doc_id)][1]}
					for doc_id in I[b_idx]
				]
	...
	processes = ProcessPool(processes=args.num_workers, initializer=init)
	get_score_partial = partial(get_score, topk=args.topk)
	...
	if args.save_pred != "":
		...
		with open(
			"/private/home/xwhan/data/nq-dpr/results/" + args.save_pred, "w"
		) as g:
			for l in to_save:
	...
	for k in aggregate:
		results = aggregate[k]
		print(
			"Top {} Recall for {} QA pairs: {} ...".format(
