...
if __name__ == "__main__":
	...
	if args.hnsw:
		if path.exists("data/hotpot_index/wiki_index_hnsw.index"):
			...
			for i in tqdm(range(0, n, buffer_size)):
				...
				hnsw_vectors = [
					np.hstack((doc_vector, aux_dims[idx].reshape(-1, 1)))
					for idx, doc_vector in enumerate(vectors)
				]
	...
	for b_start in tqdm(range(0, len(questions), args.batch_size)):
		with torch.no_grad():
			...
			batch_q_sp_encodes = tokenizer.batch_encode_plus(
				query_pairs,
			...
			for idx in range(bsize):
				...
				for _ in range(args.topk):
					...
					path_titles.append(
						[id2doc[str(hop_1_id)]["title"], id2doc[str(hop_2_id)]["title"]]
					)
				...
				if args.only_eval_ans:
					...
					for p in paths:
						concat_p += " ".join(
							[
								id2doc[doc_id]["title"] + " " + id2doc[doc_id]["text"]
								for doc_id in p
							]
						)
					metrics.append(
						{
							...
							"ans_recall": int(
								para_has_answer(
									gold_answers, concat_p, simple_tokenizer
								)
							),
	...
	if args.only_eval_ans:
		...
		for t in type2items.keys():
			...
			logger.info(
				f"Ans Recall: {np.mean([m['ans_recall'] for m in type2items[t]])}"
			)
	else:
		...
		for t in type2items.keys():
			...
			logger.info(
				f"\tAvg 1-Recall: {np.mean([m['recall_1'] for m in type2items[t]])}"
			)
			logger.info(
				f"\tPath Recall: {np.mean([m['path_covered'] for m in type2items[t]])}"
			)
