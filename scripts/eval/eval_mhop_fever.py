...
if __name__ == "__main__":
			...
			for idx in range(bsize):
				...
				ranked_pairs = np.vstack(
					np.unravel_index(
						...
						(args.beam_size_1, args.beam_size_2),
					)
				).transpose()
				retrieved_titles = []
				hop1_titles = []
				...
				for _ in range(args.topk):
					...
					path_titles.append(
						[id2doc[str(hop_1_id)][0], id2doc[str(hop_2_id)][0]]
					)
					paths_both_are_intro.append(
						id2doc[str(hop_1_id)][2] and id2doc[str(hop_2_id)][2]
					)
				...
				if args.save_path != "":
					...
					for path in path_titles:
						candidaite_chains.append(
							[
								(path[0], title2doc[path[0]]),
								(path[1], title2doc[path[1]]),
							]
						)
	...
	if args.save_path != "":
		with open(
			f"/private/home/xwhan/data/fever/retrieval/{args.save_path}", "w"
		) as out:
