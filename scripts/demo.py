...
parser.add_argument("--indexpath", type=str, default="data/hotpot_index/wiki_index.npy")
parser.add_argument(
	"--corpus_dict", type=str, default="data/hotpot_index/wiki_id2doc.json"
)
...
if query:
	...
	with torch.no_grad():
		...
		reader_input = [{"_id": 0, "question": query, "candidate_chains": chains}]
		...
		st.markdown(
			f"> **{titles_pred[0]}**: {topk_docs[titles_pred[0]].replace(answer_pred, '**' + answer_pred + '**')}"
		)
		st.markdown(
