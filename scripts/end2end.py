...
if __name__ == "__main__":
	...
	parser.add_argument(
		"--indexpath", type=str, default="retrieval/index/wiki_index_hnsw_roberta"
	)
	parser.add_argument(
		"--corpus_dict", type=str, default="retrieval/index/hotpotQA_corpus_dict.json"
	)
	parser.add_argument(
		"--retriever_path",
		type=str,
		default="retrieval/logs/08-16-2020/roberta_momentum_freeze_k-seed16-bsz150-fp16True-lr1e-05-decay0.0-warm0-valbsz3000-m0.999-k76800/checkpoint_q_best.pt",
	)
	parser.add_argument(
		"--reader_path",
		type=str,
		default="qa/logs/08-10-2020/electra_val_top30-epoch7-lr5e-05-seed42-rdrop0-qadrop0-decay0-qpergpu2-aggstep8-clip2-evalper250-evalbsize1024-negnum5-warmup0.1-adamTrue-spweight0.025/checkpoint_best.pt",
	)
	...
	with torch.no_grad():
		for b_start in tqdm(range(0, len(questions), args.batch_size)):
			...
			batch_q_encodes = retrieval_tokenizer.batch_encode_plus(
				batch_q,
			...
			# 2-hop retrieval
			s1 = time.time()
			batch_q_sp_encodes = retrieval_tokenizer.batch_encode_plus(
				query_pairs,
			...
			q_sp_embeds = retriever.encode_q(
				batch_q_sp_encodes["input_ids"],
				batch_q_sp_encodes["attention_mask"],
				batch_q_sp_encodes.get("token_type_ids", None),
			)
	...
	qa_eval_dataset = QAEvalDataset(
		qa_tokenizer, retrieval_results, max_seq_len=512, max_q_len=64
	)
	...
	ems = [
		exact_match_score(qa_results["answer"][k], id2gold_ans[k])
		for k in qa_results["answer"].keys()
	]
	f1s = [
		f1_score(qa_results["answer"][k], id2gold_ans[k])
		for k in qa_results["answer"].keys()
	]
