...
def main():
	...
	if args.do_train:
		...
		if args.use_adam:
			optimizer = Adam(
				optimizer_parameters, lr=args.learning_rate, eps=args.adam_epsilon
			)
		else:
			optimizer = AdamW(
				optimizer_parameters, lr=args.learning_rate, eps=args.adam_epsilon
	...
	if args.do_train:
		...
		train_dataset = QADataset(
			tokenizer, args.train_file, args.max_seq_len, args.max_q_len, train=True
		)
			...
			for batch in tqdm(train_dataloader):
				...
				if (batch_step + 1) % args.gradient_accumulation_steps == 0:
					...
					if args.eval_period != -1 and global_step % args.eval_period == 0:
						...
						em = metrics["em"]
						logger.info(
							"Step %d Train loss %.2f em %.2f on epoch=%d"
							% (global_step, train_loss_meter.avg, em * 100, epoch)
						)
						if best_em < em:
							logger.info(
								"Saving model with best em %.2f -> em %.2f on step=%d"
...
def predict(args, model, eval_dataloader, logger, fixed_thresh=None):
	...
	best_em, best_f1, best_joint_em, best_joint_f1, best_sp_em, best_sp_f1 = (
		0,
		0,
		0,
		0,
	...
	for lambda_ in lambdas:
		...
		for qid, res in id2result.items():
			...
			ans_res.sort(
				key=lambda x: lambda_ * x["rank_score"]
				+ (1 - lambda_) * x["span_score"],
				reverse=True,
			)
...
def eval_final(args, model, eval_dataloader, weight=0.8, gpu=True):
	...
	for batch in tqdm(eval_dataloader):
		batch_to_feed = (
			move_to_cuda(batch["net_inputs"]) if gpu else batch["net_inputs"]
		)
		batch_qids = batch["qids"]
		with torch.no_grad():
			start = time.time()
	...
	for qid in id2answer.keys():
		ans_res = id2answer[qid]
		ans_res.sort(
			key=lambda x: lambda_ * x["rank_score"] + (1 - lambda_) * x["span_score"],
			reverse=True,
		)
		top_pred = ans_res[0]["pred_str"]
		top_pred_sp = ans_res[0]["pred_sp"]
