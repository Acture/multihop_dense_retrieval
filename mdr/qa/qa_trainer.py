...
class Trainer:
	...
	def _init_state(self) -> None:
		...
		batch_size_per_gpu = (
			1 + self._train_cfg.neg_num
		) * self._train_cfg.num_q_per_gpu
		...
		self._train_loader = torch.utils.data.DataLoader(
			train_set,
			batch_size=batch_size_per_gpu * n_gpu,
			num_workers=self._train_cfg.num_workers,
			collate_fn=collate_fc,
			sampler=train_sampler,
		)
		test_set = RankingDataset(
			tokenizer,
			self._train_cfg.predict_file,
			self._train_cfg.max_seq_len,
			self._train_cfg.max_q_len,
		)
		...
		if self._train_cfg.use_adam:
			optimizer = optim.Adam(
				optimizer_parameters, lr=self._train_cfg.learning_rate
			)
		else:
		...
		t_total = (
			...
			// self._train_cfg.gradient_accumulation_steps
			* self._train_cfg.num_train_epochs
		)
		...
		self.tb_logger = SummaryWriter(
			self._train_cfg.output_dir.replace("logs", "tflogs")
		)

		checkpoint_fn = osp.join(
			self._train_cfg.output_dir, str(job_env.job_id), "checkpoint.pth"
		)
		...
		for epoch in range(start_epoch, self._train_cfg.num_train_epochs):
			...
			for batch in self._train_loader:
				...
				if (batch_step + 1) % self._train_cfg.gradient_accumulation_steps == 0:
					...
					self.tb_logger.add_scalar(
						"batch_train_loss", loss.item(), global_step
					)
					self.tb_logger.add_scalar(
						"smoothed_train_loss", train_loss_meter.avg, global_step
					)
					if job_env.global_rank == 0:
						if (
							self._train_cfg.eval_period != -1
							and global_step % self._train_cfg.eval_period == 0
						):
							metrics = self._eval()
							...
							if best_metric < score:
								print(
									"Saving model with best %s %.2f -> em %.2f"
									% (
										self._train_cfg.final_metric,
										best_metric * 100,
										score * 100,
									),
									flush=True,
			...
			if job_env.global_rank == 0:
				...
				if best_metric < score:
					...
					torch.save(
						self._state.model.state_dict(),
						os.path.join(self._train_cfg.output_dir, f"checkpoint_best.pt"),
					)
					best_metric = score
				self.log(
					{
	...
	def _eval(self) -> dict:
		...
		for batch in tqdm(eval_dataloader):
			...
			with torch.no_grad():
				...
				sp_scores = (
					sp_scores.float()
					.masked_fill(batch_to_feed["sent_offsets"].eq(0), float("-inf"))
					.type_as(sp_scores)
				)
			...
			span_mask = np.tril(
				np.triu(np.ones((max_seq_len, max_seq_len)), 0), args.max_ans_len
			)
			...
			span_scores_masked = (
				span_scores.float()
				.masked_fill((1 - span_mask[None].expand_as(span_scores)).bool(), -1e10)
				.type_as(span_scores)
			)
			start_position = span_scores_masked.max(dim=2)[0].max(dim=1)[1]
			end_position = (
				span_scores_masked.max(dim=2)[1]
				.gather(1, start_position.unsqueeze(1))
				.squeeze(1)
			)
			...
			for idx, qid in enumerate(batch_qids):
				...
				pred_str = get_final_text(
					tok_text, orig_text, do_lower_case=True, verbose_logging=False
				)

				pred_sp = []
				passages = batch["passages"][idx]
				for passage, sent_offset in zip(
					passages, [0, len(passages[0]["sents"])]
				):
				...
				id2answer[qid].append(
					(pred_str.strip(), rank_score, span_score, pred_sp)
				)
		...
		for lambda_ in lambdas:
			...
			for qid, res in id2result.items():
				...
				ans_res.sort(
					key=lambda x: lambda_ * x[1] + (1 - lambda_) * x[2], reverse=True
				)
			...
			print(
				f"joint em: {np.mean(joint_ems)}, count: {len(joint_ems)}", flush=True
			)
			print(
				f"joint f1: {np.mean(joint_f1s)}, count: {len(joint_f1s)}", flush=True
			)
		...
		return {
			"em": best_em,
			"f1": best_f1,
			"joint_em": best_joint_em,
			"joint_f1": best_joint_f1,
		}
