...
class QAEvalDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		item["encodings"] = self.tokenizer.encode_plus(
			q_toks,
			text_pair=item["wp_tokens"],
		...
		for idx, s in enumerate(item["context_processed"]["sent_starts"]):
			...
			assert item["encodings"]["input_ids"].view(-1)[
				s + para_offset
			] == self.tokenizer.convert_tokens_to_ids("[unused1]")
...
class QADataset(Dataset):
	def __init__(
		...
		if train:
				...
				if question_type == "bridge":
					ans_titles = set(
						[
							...
							for p in item["sp"]
							if para_has_answer(
								item["answer"], "".join(p["sents"]), self.simple_tok
							)
						]
					)
				else:
			...
			for item in retriever_outputs:
				...
				# for validation, add target predictions
				sp_titles = (
					set([_["title"] for _ in item["sp"]]) if "sp" in item else None
				)
				gold_answer = item.get("answer", [])
				sp_gold = []
				if "sp" in item:
	...
	def __getitem__(self, index):
		...
		if self.train:
			...
			if item["ans_covered"]:
				if item["gold_answer"][0] == "yes":
					...
					matched_spans = match_answer_span(
						context_ann["context"], item["gold_answer"], self.simple_tok
					)
					ans_starts, ans_ends = [], []
					for span in matched_spans:
						char_starts = [
							i
							for i in range(len(context_ann["context"]))
							if context_ann["context"].startswith(span, i)
						]
			...
			if item["label"]:
				assert len(item["sp_sent_labels"]) == len(
					item["context_processed"]["sent_starts"]
				)
		else:
		...
		if self.train:
			item["sent_labels"] = (
				sent_labels if len(sent_labels) != 0 else [0] * len(sent_offsets)
			)
...
def qa_collate(samples, pad_id=0):
	...
	batch = {
		"input_ids": collate_tokens(
			[s["encodings"]["input_ids"] for s in samples], pad_id
		),
		"attention_mask": collate_tokens(
			[s["encodings"]["attention_mask"] for s in samples], 0
		),
	...
	if "token_type_ids" in samples[0]["encodings"]:
		batch["token_type_ids"] = collate_tokens(
			[s["encodings"]["token_type_ids"] for s in samples], 0
		)
