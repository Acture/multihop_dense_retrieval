...
def decomposed_errors():
	top1_pred = [
		json.loads(l)
		for l in open(
			"/private/home/xwhan/data/hotpot/dense_val_b1_top1.json"
		).readlines()
	]
...
def collect_gold_decomposition():
	...
	dev_qdmr = [
		json.loads(l)
		for l in open("/private/home/xwhan/data/QDMR/dev.json").readlines()
	]
...
def qdmr_utils():
	...
	qdmr_data = [
		json.loads(l)
		for l in open("/private/home/xwhan/data/QDMR/inspect.json").readlines()
	]
	...
	for idx, item in enumerate(qdmr_data):
		...
		mhop_data.append(
			{"question": question, "sp": sp, "type": "bridge", "_id": item["id"]}
		)
...
def analyze_results():
	decomposed_results = [
		json.loads(l)
		for l in open("/private/home/xwhan/data/QDMR/qdmr_decomposed_results.json")
	]
	e2e_results = [
		json.loads(l)
		for l in open("/private/home/xwhan/data/QDMR/qdmr_e2e_results.json")
	]
	better = 0
	worse = 0
	both = 0
