...
def mhop_loss(model, batch, args):
	...
	neg_ctx = torch.cat(
		[outputs["neg_1"].unsqueeze(1), outputs["neg_2"].unsqueeze(1)], dim=1
	)  # B x 2 x M x h

	scores_1_hop = torch.mm(outputs["q"], all_ctx.t())
	neg_scores_1 = torch.bmm(
		outputs["q"].unsqueeze(1), neg_ctx.transpose(1, 2)
	).squeeze(1)
	scores_2_hop = torch.mm(outputs["q_sp1"], all_ctx.t())
	neg_scores_2 = torch.bmm(
		outputs["q_sp1"].unsqueeze(1), neg_ctx.transpose(1, 2)
	).squeeze(1)

	# mask the 1st hop
	bsize = outputs["q"].size(0)
	scores_1_mask = torch.cat([torch.zeros(bsize, bsize), torch.eye(bsize)], dim=1).to(
		outputs["q"].device
	)
	scores_1_hop = (
		scores_1_hop.float()
		.masked_fill(scores_1_mask.bool(), float("-inf"))
	...
	if args.momentum:
		queue_neg_scores_1 = torch.mm(
			outputs["q"], model.module.queue.clone().detach().t()
		)
		queue_neg_scores_2 = torch.mm(
			outputs["q_sp1"], model.module.queue.clone().detach().t()
		)
	...
	target_2_hop = torch.arange(outputs["q"].size(0)).to(outputs["q"].device) + outputs[
		"q"
	].size(0)

	retrieve_loss = loss_fct(scores_1_hop, target_1_hop) + loss_fct(
		scores_2_hop, target_2_hop
	)
...
def unified_loss(model, batch, args):
	...
	stop_loss = F.cross_entropy(
		outputs["stop_logits"], batch["stop_targets"].view(-1), reduction="sum"
	)
	...
	retrieve_loss = (
		F.cross_entropy(scores_1_hop, target_1_hop, reduction="sum")
		+ (
			F.cross_entropy(scores_2_hop, target_2_hop, reduction="none")
			* batch["stop_targets"].view(-1)
		).sum()
	)
...
def unified_eval(outputs, batch):
	...
	for t1, idx2ranked1, t2, idx2ranked2, stop in zip(
		target_1_hop, idx2ranked_1, target_2_hop, idx2ranked_2, stop_targets
	):
		if stop:  #
