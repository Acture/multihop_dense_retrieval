...
def pick_bridge_v0(title2linked, title2doc, titles, q, ans):
	...
	"""
	# check answer
	if (ans in titles[0] + " " + title2doc[titles[0]]) and ans not in titles[
		1
	] + " " + title2doc[titles[1]]:
		return titles[0]
	elif (ans in titles[1] + " " + title2doc[titles[1]]) and (
		ans not in titles[0] + " " + title2doc[titles[0]]
	):
		return titles[1]
	elif (
		titles[0] in title2linked[titles[1]]
		and titles[1] not in title2linked[titles[0]]
	):
		return titles[0]
	else:
		return titles[1]
		...
		for item in tqdm(split):
			...
			if item["type"] == "comparison":
				...
				for title in sp_titles:
					pos_paras.append(
						{"title": title, "text": "".join(title2passage[title])}
					)
				data_to_save.append(
					{
						"question": question,
				...
				bridge = pick_bridge(
					title2linked, title2doc, sp_titles, question, item["answer"]
				)
	...
	for s, raw_data in zip(["train", "val"], [raw_train, raw_val]):
		...
		for item in raw_data:
			...
			if _type == "bridge":
				...
				bridge_title = pick_bridge_v0(
					title2linked, title2doc, sp, question, answer[0]
				)
	...
	for split, raw in zip(["train", "val"], [raw_train, raw_val]):
		...
		for idx, item in enumerate(data):
			...
			if item["type"] == "bridge":
				...
				bridge_title = pick_bridge_v0(
					title2linked, title2doc, sp_titles, item["question"], ans
				)
...
def check_2hop(raw_path):
	...
	target_in_query = [
		int(
			item["pos_para"]["title"].strip() in item["sp1"]
			or item["pos_para"]["title"].lower().strip() in item["sp1"]
		)
		for item in data
	]
...
def add_sp_labels(
	raw_path, input_file, save_path, title2sent_map="data/hotpot_index/title2sents.txt"
):
	"""
				...
				instance["sp"].append(
					{
						...
						"sp_sent_ids": sptitle2sentids[title],
					}
				)
...
def explore_QDMR(path="/private/home/xwhan/data/Break-dataset/QDMR-high-level"):
	...
	hotpot_retrieval_train = [
		json.loads(l)
		for l in open(
			"/private/home/xwhan/data/hotpot/hotpot_train_with_neg_v0.json"
		).readlines()
	]
	hotpot_retrieval_val = [
		json.loads(l)
		for l in open(
			"/private/home/xwhan/data/hotpot/hotpot_val_with_neg_v0.json"
		).readlines()
	]
	qid2data = {}
...
def add_sents_to_corpus_dict():
	id2doc = json.load(
		open("/private/home/xwhan/Mhop-Pretrain/retrieval/index/abstracts_id2doc.json")
	)
	title_and_sents = [
		json.loads(l)
		for l in open(
			"/private/home/xwhan/data/hotpot/tfidf/title_sents.txt"
		).readlines()
	]
	title2sents = {_["title"]: _["sents"] for _ in title_and_sents}
	...
	for k in id2doc.keys():
		...
		sents = title2sents[title]
		id2doc[k] = {"title": title, "text": text, "sents": sents}

	json.dump(
		id2doc,
		open(
			"/private/home/xwhan/Mhop-Pretrain/retrieval/index/hotpotQA_corpus_dict.json",
			"w",
		),
	)
