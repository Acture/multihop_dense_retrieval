...
def convert_to_half(sample):
	...
	def _convert_to_half(maybe_floatTensor):
		if (
			torch.is_tensor(maybe_floatTensor)
			and maybe_floatTensor.type() == "torch.FloatTensor"
		):
			return maybe_floatTensor.half()
		elif isinstance(maybe_floatTensor, dict):
			return {
				key: _convert_to_half(value) for key, value in maybe_floatTensor.items()
			}
...
def complex_ans_recall():
	...
	predictions = json.load(
		open(
			"/private/home/xwhan/code/learning_to_retrieve_reasoning_paths/results/complexwebq_retrieval_res.json"
		)
	)
	raw_dev = [
		json.loads(l)
		for l in open(
			"/private/home/xwhan/data/ComplexWebQ/complexwebq_dev_qas.txt"
		).readlines()
	]
